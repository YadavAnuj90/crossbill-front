...
export default function InvoiceDetailPage() {
  ...
  async function getPdf() {
    ...
    setBusy(true);
    try {
      const { url } = await api.invoices.pdf(inv.id);
      window.open(url, '_blank');
    } catch {
      notify('info', 'PDF is still generating — try again in a moment');
    } finally { setBusy(false); }
  }
  ...
  return (
    <div>
      ...
      <PageHeader
        title={inv.number}
        subtitle={`Raised ${formatDate(inv.invoiceDate)} · FY ${inv.financialYear}`}
        action={
      ...
      <div className="grid gap-6 lg:grid-cols-[1.6fr_1fr]">
        ...
        <Card className="h-fit">
          <CardHeader title={<span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-brand-600" /> Compliance</span>} />
