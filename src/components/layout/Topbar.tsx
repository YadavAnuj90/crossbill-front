      ...
      <div className="flex items-center gap-3 px-4 sm:px-5 py-2.5">
        ...
        <div className="group relative hidden sm:block w-full max-w-sm">
          ...
          <input
            ...
            className="w-full rounded-xl border border-paper-border bg-paper/70 pl-9 pr-16 py-2 text-sm text-ink placeholder:text-ink-faint outline-none transition-all focus:border-brand-400 focus:bg-white focus:shadow-[0_0_0_4px_rgba(16,185,129,0.10)]"
          />
          <span className="absolute right-2.5 top-1/2 -translate-y-1/2 hidden items-center gap-0.5 rounded-md border border-paper-border bg-white px-1.5 py-0.5 text-[10px] font-medium text-ink-faint group-focus-within:hidden sm:flex">
            <Command className="h-2.5 w-2.5" />K
          </span>
        </div>

        <div className="ml-auto flex items-center gap-2">
          <span className="hidden lg:inline-flex items-center gap-1.5 rounded-full border border-brand-200/60 bg-brand-50/60 px-3 py-1.5 text-xs font-medium text-brand-700">
            <ShieldCheck className="h-3.5 w-3.5" /> Export-compliant
          </span>

          {/* Notifications */}
          <div className="relative">
            <button onClick={() => { setBell((b) => !b); setMenu(false); }} className={cn('relative grid h-9 w-9 place-items-center rounded-xl border bg-white/70 transition', bell ? 'border-brand-300 text-brand-600' : 'border-paper-border text-ink-muted hover:text-ink hover:bg-paper')} aria-label="Notifications">
          ...
          {/* Profile */}
          <div className="relative">
            <button onClick={() => { setMenu((m) => !m); setBell(false); }} className={cn('flex items-center gap-2 rounded-xl border px-1.5 py-1 transition', menu ? 'border-paper-border bg-white' : 'border-transparent hover:border-paper-border hover:bg-white/70')}>
              <span className="relative">
                <span className="grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-brand-400 to-brand-600 text-white text-xs font-semibold ring-2 ring-white">{initials(name)}</span>
              </span>
            ...
            {menu && (
              <>
                ...
                <div className="card absolute right-0 mt-2 w-64 z-20 shadow-lift py-1.5 overflow-hidden animate-fade-in">
                  <div className="px-3.5 py-3 border-b border-paper-border bg-gradient-to-br from-paper to-white">
                    ...
                    <span className="mt-2.5 inline-flex items-center gap-1.5 rounded-full bg-brand-50 px-2 py-0.5 text-[11px] font-medium text-brand-700"><Sparkles className="h-3 w-3" /> Free plan</span>
                  </div>
                  ...
                  <button onClick={() => { setMenu(false); logout(); }} className="flex w-full items-center gap-2.5 px-3.5 py-2 text-sm text-red-600 hover:bg-red-50 transition-colors"><LogOut className="h-4 w-4" /> Sign out</button>
                </div>
              </>
            )}
          </div>
