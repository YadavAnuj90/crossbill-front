'use client';
import { Check, Sparkles, Crown, Users, Settings as SettingsIcon } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  return (
    <div>
      <PageHeader eyebrow="Setup" title="Settings" subtitle="Plan, billing and account." icon={<SettingsIcon className="h-5 w-5" />} />
      <Reveal>
        <Card className="mb-6">
          ...
          <CardBody>
            <div className="grid gap-4 md:grid-cols-3">
              {PLANS.map((p) => {
                const Icon = p.icon;
                return (
                  <div key={p.id} className={cn('rounded-2xl border p-5 relative transition', p.highlight ? 'border-brand-400 bg-brand-50/40 shadow-card' : 'border-paper-border')}>
